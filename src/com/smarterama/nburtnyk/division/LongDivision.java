...
public class LongDivision {
	...
	public List<String> buildOutputStrings() {
		List<DivisionStep> steps = buildLongDivisionStepsList();
	...
	protected List<DivisionStep> buildLongDivisionStepsList() {
		List<DivisionStep> longDivisionStepsList = new ArrayList<DivisionStep>();
		...
		while (currentDividend >= Math.abs(divisor)) {
			...
			longDivisionStepsList.add(step);
		}
		if (result.length() > 0) {
			DivisionStep lastStep = longDivisionStepsList.get(longDivisionStepsList.size() - 1);
